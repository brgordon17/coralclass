#' Function to perform PCA for NMR data.
#'
#' \code{nmrpca()} was used to perform the Principal Components Analysis of the
#' NMR data (\code{./data/nmrdata.rda})
#'
#' \code{nmrpca()} zero-centres the data and performs a PCA using singular value
...
#' \code{./inst/extdata/nmrpca_ggobject.rds}.
...
nmrpca <- function(scale = FALSE,
                   ...
                   save.png = FALSE,
                   save.gg = FALSE,
                   plot.name = "nmrpca_plot",
                   seed = 1978,
                   ...) {

  set.seed(seed)
  pca <- stats::prcomp(nmrdata[-1:-6], scale = scale, center = center)
  ...
  if (save.gg) {
    saveRDS(pcaplot, "./inst/extdata/nmrpca_ggobject.rds")
  }
